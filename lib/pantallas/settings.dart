...
class settings extends StatelessWidget {
  ...
  void _onDelete(String nombre, double valor){
    cuenta.value.fijos.removeWhere((element) => element.nombre == nombre && element.valor == valor);
  }

  void _onChange(String nombre, double valor){
    cuenta.value.fijos.where((element) => element.nombre == nombre).first.valor = valor;
  }
  ...
  Widget build(BuildContext context) {
    return Obx(()=>PopScope(
      ...
      child: Scaffold(
        floatingActionButton: crearNuevo(onTap: ()=>Values().gastoSeleccionado.value = -2),
          body: Center(
            child: Padding(
              padding: EdgeInsets.all(15),
              child: Column(children: [
                fijosView(gastos: cuenta.value.fijos, onDelete: _onDelete, onChange: _onChange, theme: Theme.of(context)),
                Values().gastoSeleccionado.value == -2
                  ? nuevoFijo(onCreate: _onCreate, theme: Theme.of(context))
                  : noFijos()
              ],)
              ),
            ),
